...
list(
  ...
  tar_target(fit_rasch_binomial_partisans_colorado_adams, 
             fit_binomial(data_colorado_adams, type = "rasch"),
             format = "qs"),
  tar_target(fit_2pl_binomial_partisans_colorado_adams, 
             fit_binomial(data_colorado_adams, type = "2pl"),
             format = "qs"),
  tar_target(fit_rasch_binomial_partisans_colorado,
             fit_binomial(data_colorado, type = "rasch"),
             format = "qs"),
  tar_target(stan_data, get_stan_data(data_base_adams)),
  tar_target(stan_rasch_code, "R/cat_1pl.stan", format = "file"),
  tar_target(stan_2pl_code, "R/cat_2pl.stan", format = "file"),
  tar_target(fit_rasch_categorical_colorado, fit_stan(stan_rasch_code, stan_data, "cat_1pl")),
